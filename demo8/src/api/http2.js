...
const http = function http(config) {
  ...
  // 处理问号传参
  if (params) {
    url += `${url.includes('?') ? '&' : '?'}${qs.stringify(params)}`; // 'xxx=xxx&xxx=xxx'
  }
  //'/api/xxx? 看url有没有?号，有?加&再传参，因为前面url已经包含了参数了肯定'

  // 处理请求主体信息：按照我们后台要求，如果传递的是一个普通对象，我们要把其设置为urlencoded格式「设置请求头」？每个项目不一样
  if (_.isPlainObject(body)) {
    // 只是本项目。因为本项目不是json类型的传参
